...
.PHONY: sno-restore
sno-restore: ## Restore SNO vms from snapshot
	ansible-playbook -i hosts $(TAGS_STRING) --extra-vars='{"snos":[$(SNOS)]}' $(EXTRA_VARS) playbooks/sno-restore.yml
...
.PHONY: mcg-qe
mcg-qe: ## Install multicloud gitops on two snos (sno1 and sno2 by default)
	ansible-playbook -i hosts $(TAGS_STRING) --extra-vars='{"snos":[$(SNOS)]}' $(EXTRA_VARS) playbooks/sno-mcg-qe.yml
